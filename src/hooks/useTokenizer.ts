...
const useTokenizer = () => {
  ...
  const encoder = useCallback(async (textToEncode: string): Promise<number[]> => {
    switch (tokenizerID) {
      case 'cl100k_base':{
        const { encode: cl100kEncode } = await import('gpt-tokenizer/encoding/cl100k_base')
        return cl100kEncode(textToEncode)
      }
      case 'o200k_base':{
        const { encode: o200kEncode } = await import('gpt-tokenizer/encoding/o200k_base')
